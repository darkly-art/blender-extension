"""Tests for `encode.frame_is_duplicate` - the pre-encode raw-frame dedup.
...
what drops redundant frames before the expensive PNG encode. It keys on the raw
scene-linear buffer plus its `ViewSettings`, so it never drops a distinct-
looking frame, yet correctly refuses to drop a grading-only change (same pixels,
different display transform).
"""
...
class FrameDedupTest(unittest.TestCase):
    ...
    def test_identical_raw_and_settings_is_duplicate(self):
        self.assertTrue(
            encode.frame_is_duplicate(self.raw.copy(), self.vs, self.raw, self.vs)
        )
    ...
    def test_changed_pixels_is_not_duplicate(self):
        ...
        self.assertFalse(encode.frame_is_duplicate(changed, self.vs, self.raw, self.vs))

    def test_resize_is_not_duplicate(self):
        resized = np.zeros((5, 6, 4), np.float32)
        self.assertFalse(encode.frame_is_duplicate(resized, self.vs, self.raw, self.vs))

    def test_first_frame_is_not_duplicate(self):
        self.assertFalse(encode.frame_is_duplicate(self.raw, self.vs, None, None))
    ...
    def test_camera_source_none_settings_dedups_on_pixels(self):
        ...
        self.assertTrue(encode.frame_is_duplicate(raw.copy(), None, raw, None))
        changed = raw.copy()
        changed[0, 0, 0] ^= 0xFF
        self.assertFalse(encode.frame_is_duplicate(changed, None, raw, None))
    ...
    def test_settings_presence_mismatch_is_not_duplicate(self):
        ...
        self.assertFalse(
            encode.frame_is_duplicate(self.raw.copy(), self.vs, self.raw, None)
        )
